...
# catdefs cat's out all the definitions files relevant to a task.
...
catdefs (){
	...
	[ -e "$RCROOT/defaultsite/defs/${RCTASK}.defs" ] && cat "$RCROOT/defaultsite/defs/${RCTASK}.defs" || :
	# ... then definitions from sites/common
	[ -e "$RCROOT/sites/common/defs/${RCTASK}.defs" ] && cat "$RCROOT/sites/common/defs/${RCTASK}.defs" || :
	# Now check for site task definitions
	[ -e "$RCROOT/sites/$RCSITE/defs/${RCTASK}.defs" ] && cat "$RCROOT/sites/$RCSITE/defs/${RCTASK}.defs" || :
	# ... and host task definitions
	[ -e "$RCROOT/sites/$RCSITE/hosts/$RCHOST/defs/${RCTASK}.defs" ] && cat "$RCROOT/sites/$RCSITE/hosts/$RCHOST/defs/${RCTASK}.defs" || :
